...
async function testMidiTemplate() {
    try {
        console.log('🧪 === TEST TEMPLATE VÉRIFICATION MIDI ===\n');
        ...
        if (userEmail) {
            ...
            if (users.length === 0) {
                ...
                console.log('\n💡 Utilisation:');
                console.log('   node scripts/test-midi-template.js email@example.com');
                return;
            }
        ...
        // Construire le contenu (variables du template)
        console.log('\n🔧 Génération des statistiques de la matinée...');
        const noonData = await NotificationContentBuilder.buildNoonContent(user.id);
        
        console.log('\n📝 === CONTENU DES VARIABLES ===');
        console.log('==========================================');
        console.log(`Variable {{1}} - Tâches accomplies: ${noonData.var1}`);
        console.log(`Variable {{2}} - Temps de travail: ${noonData.var2}`);
        console.log('==========================================');
        ...
        if (useTemplates) {
            ...
            console.log(`   - Template: productif_verification_midi`);
            console.log(`   - Variable {{1}}: ${noonData.var1}`);
            console.log(`   - Variable {{2}}: ${noonData.var2}`);
            ...
            try {
                ...
                let fallbackMessage = `🍽 Pause déjeuner bien méritée\n\n`;
                fallbackMessage += `🕛 C'est l'heure de la pause déjeuner !\n\n`;
                fallbackMessage += `📊 Bilan de la matinée :\n`;
                fallbackMessage += `✅ ${noonData.var1} tâches accomplies\n`;
                fallbackMessage += `⏱ ${noonData.var2} de travail\n\n`;
                fallbackMessage += `💭 Comment s'est passée ta matinée ?\n\n`;
                fallbackMessage += `🍽 Bonne pause déjeuner ! On se retrouve après manger`;
            ...
            let textMessage = `🍽 Pause déjeuner bien méritée\n\n`;
            textMessage += `🕛 C'est l'heure de la pause déjeuner !\n\n`;
            textMessage += `📊 Bilan de la matinée :\n`;
            textMessage += `✅ ${noonData.var1} tâches accomplies\n`;
            textMessage += `⏱ ${noonData.var2} de travail\n\n`;
            textMessage += `💭 Comment s'est passée ta matinée ?\n\n`;
            textMessage += `🍽 Bonne pause déjeuner ! On se retrouve après manger`;
        ...
        await prisma.notificationHistory.create({
            data: {
                userId: user.id,
                type: 'NOON_CHECK',
                content: `${noonData.var1} tâches - ${noonData.var2}`,
