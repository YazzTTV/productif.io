...
async function testVerificationNuitTemplate() {
    try {
        console.log('🧪 === TEST TEMPLATE VÉRIFICATION NUIT ===\n');
        ...
        if (userEmail) {
            ...
            if (users.length === 0) {
                ...
                console.log('\n💡 Utilisation:');
                console.log('   node scripts/test-verification-nuit-template.js email@example.com');
                return;
            }
        ...
        const nightMessage = await NotificationContentBuilder.buildNightContent(user.id);
        ...
        await prisma.notificationHistory.create({
            data: {
                userId: user.id,
                type: 'NIGHT_CHECK',
                content: nightMessage.substring(0, 200) + '...',  // Tronquer si trop long
