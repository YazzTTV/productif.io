...
export function TasksAwareness({ tasks: initialTasks, onContinue, t }: TasksAwarenessProps) {
  ...
  const toggleRecording = () => {
    if (isRecording) {
      ...
      // In a real app, would process speech-to-text here
      // Mock: append some text
      setTasks(prev => prev + '\n[Voice transcription would appear here]');
    } else {
      // Start recording
      setIsRecording(true);
      // In a real app, would start speech recognition
    }
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12">
      ...
      >
        <div className="text-center space-y-3">
          ...
          <p className="text-black/60">{t.writeOrSpeak}</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-black/40 px-1">{t.messyIsFine}</p>
        </div>
