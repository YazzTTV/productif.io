...
export class MorningInsightsScheduler {
  ...
  async sendMorningInsights() {
    try {
      ...
      for (const user of users) {
        try {
          console.log(`\n🔍 [MorningInsights] Traitement utilisateur: ${user.id}`)
          const settings = user.notificationSettings
          console.log(`   ⚙️ improvementReminder: ${settings?.improvementReminder}`)
          console.log(`   ⏰ improvementTime: ${settings?.improvementTime}`)
          
          // Respecter le flag d'activation spécifique à l'amélioration si présent
          if (settings && settings.improvementReminder === false) {
            console.log(`   ❌ SKIP: improvementReminder désactivé`)
            continue
          }
          const target = (settings?.improvementTime || '08:30').split(':')
          const [th, tm] = [target[0] || '07', target[1] || '00']
          // Comparer l'heure Paris actuelle (nowH:nowM) avec l'heure cible (th:tm)
          // Tolérance ±2 minutes
          const nowMinutes = Number(nowH) * 60 + Number(nowM)
          const targetMinutes = Number(th) * 60 + Number(tm)
          const diffMinutes = Math.abs(nowMinutes - targetMinutes)
          console.log(`   🕐 nowMinutes: ${nowMinutes}, targetMinutes: ${targetMinutes}, diff: ${diffMinutes} minutes`)
          if (diffMinutes > 2) {
            console.log(`   ❌ SKIP: Heure cible ${th}:${tm} trop éloignée (diff: ${diffMinutes} min > 2 min)`)
            continue
          }
          console.log(`   ✅ Heure correspondante (diff: ${diffMinutes} min)`)
          ...
          // DEBUG: Vérifier tous les journaux (avec et sans processed)
          const allJournals = await prisma.journalEntry.findMany({
            where: {
              userId: user.id,
              date: { gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) }
            },
            select: { id: true, date: true, processed: true, processingError: true, transcription: true },
            orderBy: { date: 'desc' }
          })
          console.log(`   🔍 DEBUG: ${allJournals.length} journaux au total (7 jours)`)
          allJournals.forEach(j => {
            console.log(`      - ${j.date.toISOString()} | processed: ${j.processed} | id: ${j.id}`)
            if (j.processingError) console.log(`        ⚠️ Erreur: ${j.processingError}`)
            if (!j.processed && j.transcription) console.log(`        ⏳ Transcription présente mais non traitée`)
          })
          ...
          console.log(`   📔 Journaux récents (processed=true): ${recentJournals} (période: ${actualPeriod} jours)`)
          
          if (recentJournals === 0) {
            console.log(`   ❌ SKIP: Aucun journal récent`)
            continue
          }
          ...
          console.log(`   📊 Insight existant: ${insight ? `Oui (id: ${insight.id}, sent: ${insight.sent})` : 'Non'}`)
          if (!insight) {
            console.log(`   🤖 Génération d'un nouvel insight...`)
            ...
            console.log(`   ✅ Insight créé: ${insight.id}`)
          }

          const phone = user.whatsappNumber || (user.notificationSettings && user.notificationSettings.whatsappNumber)
          console.log(`   📱 Téléphone: ${phone ? phone : '❌ Non configuré'}`)
          console.log(`   📧 Insight.sent: ${insight.sent}`)
          if (!insight.sent && phone) {
            console.log(`   ✉️ ENVOI WhatsApp à ${phone}...`)
            ...
            console.log(`   ✅ Insight envoyé avec succès`)
          } else {
            if (insight.sent) console.log(`   ❌ SKIP: Insight déjà envoyé`)
            if (!phone) console.log(`   ❌ SKIP: Pas de numéro WhatsApp`)
          }
        } catch (e) {
  ...
  async sendInsightToUser(phoneNumber, insight) {
    // Système de templates désactivé - envoi en texte normal
