...
export function DailyStruggles({
  ...
  const struggles = [
    { value: 'toomany', label: getTranslation('tooManyTasks', 'Trop de tâches dans ma tête') },
    { value: 'focus', label: getTranslation('difficultyFocusing', 'Difficulté à se concentrer profondément') },
    { value: 'stress', label: getTranslation('constantStress', 'Stress constant') },
    { value: 'guilt', label: getTranslation('guiltyResting', 'Se sentir coupable de se reposer') },
    { value: 'fear', label: getTranslation('fearFallingBehind', 'Peur de se laisser distancer par les autres') },
  ];
      ...
      >
        <div className="text-center space-y-3">
          <h1 className="tracking-tight text-3xl font-bold" style={{ letterSpacing: '-0.04em' }}>
            {getTranslation('dailyDifficulties', 'Quels sont les défis quotidiens ?')}
          </h1>
          <p className="text-black/40 text-sm">{getTranslation('selectMultiple', 'Sélectionnez toutes les options qui s\'appliquent')}</p>
        </div>
